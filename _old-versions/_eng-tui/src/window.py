...
class Window:
    ...
    def input(self, input_name):
        ...
        default_txt = ''.join(self.choose_char(chars) for i in range(max_len))
        self.flushinp()
        ...
        while True:
            ...
            if ch in [9,10]: # tab, enter
                if not inp:
                    self.flushinp()
                    self.cursor('off')
                    self.addstr_animate_cover(y,x, default_txt, delay=0.01,
                                              color_search='r', color_result=color_p,
                                              search_count=5, chset='xX')
                    self.cursor('on')
                else:
                    break
            elif ch == 27: # escape
    ...
    # animate
    def addstr_animate_linear(self, y,x, text, delay, color_search='d', color_result='c',
                              search_count=3, chset=False):
        for i,char in enumerate(text):
            for i in range(search_count):
                self.render_chset(y,x, color_search, delay, chset)
            self.addstr(y,x, char, color_result).refresh()
            x += 1
        return self

    def addstr_animate_cover(self, y,x, text, delay, color_search='y', color_result='c',
                             search_count=3, chset=False):
        done_chars = 0
        ...
        while done_chars < len(text):
            ...
            if matrix[random_index] < search_count:
                self.render_chset(y,x+random_index, color_search, delay, chset)
    ...
    def render_chset(self, y,x, color, delay, chset=False):
        if not chset:
            chset = string.ascii_letters + string.digits
        self.addstr(y,x, self.choose_char(chset), color).refresh()
        time.sleep(delay)
        return self
